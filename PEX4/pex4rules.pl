...
% lead_time(Activity,Time) finds the lead time of an activity. It's the
% time remaining in the critical path.
lead_time(Activity,Time):-
	activity(_,B,Activity,_),
	end(E),
	longest_path(B,E,LeadPath),
	time(LeadPath,Time).
